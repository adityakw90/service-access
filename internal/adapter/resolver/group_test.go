...
func TestGroupResolverIDsByUIDs(t *testing.T) {
	tests := []struct {
		name    string
		uids    []string
	...
	}{
		{
			...
			uids: []string{"group-uid-1"},
			setup: func(mockPool pgxmock.PgxPoolIface) {
				rows := pgxmock.NewRows([]string{"id", "uid"}).
					AddRow(int64(100), "group-uid-1")
				mockPool.ExpectQuery(`SELECT id, uid FROM "group" WHERE uid=\$1`).
					WithArgs("group-uid-1").
					WillReturnRows(rows)
			},
			want:    map[string]int64{"group-uid-1": 100},
			wantErr: false,
		},
		{
			name: "Happy Path - multiple UIDs",
			uids: []string{"group-uid-1", "group-uid-2"},
			setup: func(mockPool pgxmock.PgxPoolIface) {
				...
				rows2 := pgxmock.NewRows([]string{"id", "uid"}).
					AddRow(int64(200), "group-uid-2")
				mockPool.ExpectQuery(`SELECT id, uid FROM "group" WHERE uid=\$1`).
					WithArgs("group-uid-2").
					WillReturnRows(rows2)

				// Allow expectations to be satisfied in any order
				mockPool.MatchExpectationsInOrder(false)
			},
			want:    map[string]int64{"group-uid-1": 100, "group-uid-2": 200},
			wantErr: false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			resolver := NewGroupResolver(mockPool, redisClient, "group", 0, infra.NewNoopLogger(), infra.NewNoopTracer())
...
func TestGroupResolverUIDsByIDs(t *testing.T) {
	tests := []struct {
		name    string
		ids     []int64
		setup   func(pgxmock.PgxPoolIface)
	...
	}{
		{
			...
			want:    map[int64]string{100: "group-uid-1"},
			wantErr: false,
		},
		{
